...
```bash
k3sup join --host=NODE2 --server-user=USERNAME --server-host=VIP --user=USERNAME --server --k3s-extra-args="--disable servicelb --node-taint node-role.kubernetes.io/master=true:NoSchedule"
```
For server IP we use the VIP instead of the ip of node 1
```bash
k3sup join --host=NODE3 --server-user=USERNAME --server-host=VIP --user=USERNAME --server --k3s-extra-args="--disable servicelb --node-taint node-role.kubernetes.io/master=true:NoSchedule"
```
