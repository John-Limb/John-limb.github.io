...
We want to set the DNS servers to 1.1.1.9 and 9.9.9.9 so that cert manager can look up domains on the internet, this will prevent it matching your internal dns records and then failing the challenege.

You will need to create an API token within your cloudfront account that can read and write to your domain. 
...
kind: Certificate
metadata:
  name: staging-cert-name
  namespace: default
spec:
  secretName: staging-cert-name
  issuerRef:
    name: letsencrypt-staging
    kind: ClusterIssuer
...
spec:
  entryPoints:
    - websecure
  routes:
    - match: Host(`www.nginx.domain.tld`)
      kind: Rule
      services:
        - name: nginx
          port: 80
    - match: Host(`nginx.domain.tld`)
      kind: Rule
      services:
        - name: nginx
          port: 80
